...
/*   Updated: 2019/03/04 11:29:58 by jmeier           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	hash_slinging_slasher(t_sh *sh)
{
	...
	// ft_map_set(sh->builtin, ft_map_hash(sh->builtin, "unsetenv"), &b_unsetenv);
...
// 	hash_slinging_slasher(&sh);
	...
	while (g_running)
	{
		if ((!read_line(line, &sh)))
			continue ;
		!g_clear && line->length > 1 ? command_exe(line, &sh) :
			(line->length = 0);
		if (g_running)
			prompt();
	}
